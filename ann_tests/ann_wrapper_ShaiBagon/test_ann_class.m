...
%Supress warnings about obsolete function randint
warning_id = 'comm:system:warnobsolete:obsoleteReplace';
...
% load test
clear anno;
fprintf(1,'start load test\n');
for ii=1:6
    anno{ii} = ann(rand(100,10000));
end
for ii=1:10000
    ai = randint(1,1,6)+1;%1-6
    %ai = [randi(6)];
    [idx dst] = ksearch(anno{ai}, rand(100,1), randint(1,1,40)+10, 1.0);%10-49
    %[idx dst] = ksearch(anno{ai}, rand(100,1), [rand(40)+9], 1.0);
    if mod(ii,100)==0
